...
public class TestToStringAndUpdate {
	...
	@Test
	public void testReferceAtomicAddress() throws InterruptedException {
		try (AllInterleavings allInterleavings = new AllInterleavings("TestToStringAndUpdate_ThreadSafe_atomic");) {
			while (allInterleavings.hasNext()) {
				AtomicAdress address = new AtomicAdress("E. Bonanza St.", "South Park", "456 77 99");
				String readAddress = null;
				Thread first = new Thread(() -> {
					address.update("Evergreen Terrace", "Springfield");
				});
